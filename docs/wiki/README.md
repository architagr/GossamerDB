...
- ~~**Client surface.**~~ Resolved 2026-04-29 — **Go SDK is the only first-class client in v1.** Non-Go callers (Java, Python, Node, Rust, etc.) use the **gRPC** or **Fiber-backed REST** wire protocol directly — both surfaces are stable, mTLS-required, and 1-to-1 with each other. The smart-client features (partition-map cache, epoch-driven `WRONG_OWNER` redirect, token-aware routing — see PRD FR-20) are Go-only in v1; non-Go callers use the **any-data-node forwarding fallback** (FR-8 tier 3) and pay one extra intra-AZ hop. **Java or Python SDK in v1.1 by demand.** See PRD §11 Q14, NG-3, FR-8, FR-20.
- ~~**Observability backends.**~~ Resolved 2026-04-29 — **Full OTel + Prometheus + Tempo + Loki + Grafana stack ships as a reference deployment in v1.** GossamerDB binary exports **OTLP gRPC for all three signals** (metrics, traces, logs) — single wire format. Architecture is **OTel Collector-mediated**: GossamerDB → OTLP → OpenTelemetry Collector → fans out to Prometheus (metrics), Tempo (traces), Loki (logs); Grafana queries all three. **Prometheus scrape on `/metrics`** is kept as a secondary back-compat path for operators with existing Prom-only dashboards. Reference deployment under **`deploy/observability/`**: `docker-compose.yaml` (local dev — brings up the full stack) and a Helm sub-chart for k8s mode. Pre-provisioned Grafana datasources + a **three-panel cross-correlated dashboard** (metrics RED, request-path traces, structured logs) so operators can jump from a slow span to its logs. Sample Prometheus alert rules ship too. **Logs format = OTLP logs** (OTel-native), with stdout-JSON fallback if the Collector is unreachable. We do not redistribute upstream binaries — we ship configs and pull from upstream images at run time. See PRD §11 Q16, FR-11, FR-17, NFR-OPS-1.
- ~~**Tenancy.**~~ Resolved 2026-04-29 — **Single-tenant per cluster in v1.** One cluster = one logical workload; multi-tenancy via namespace isolation lands in v1.1 (gated on the per-namespace authz work in §9.2 #2 above). Operators that need tenant isolation today run multiple clusters. See PRD §11 Q18, NG-6.
- ~~**mTLS / PKI source of truth.**~~ Resolved 2026-04-29 — **Operator-supplied PKI in v1, exposed via a `pki.Source` interface with two implementations:** **`disk`** (cert/key paths in config; fsnotify-based hot-reload on file change) and **`k8s-secret`** (references a k8s Secret; watches the Secret API for updates). **cert-manager users get free integration via `k8s-secret`** — point cert-manager at the same Secret. **`gossamerctl dev-pki`** generates a localhost CA + node + admin + client certs as a one-shot for **local dev only** (refuses to run in non-local mode). **Hot-reload semantics:** new certs apply at the next handshake; in-flight TLS sessions keep their old creds until they naturally close — zero downtime. **Cert overlap window default 24 h**; `mtls_cert_expires_in_seconds` exported as a Prometheus metric with a Grafana panel + sample alert. **Cert reload events written to the audit log (FR-16).** **Identity scheme:** SAN-based with cluster name embedded — `coordinator.<cluster>`, `data.<cluster>.<region>`, `admin.<cluster>`, `client.<cluster>` — preventing cross-cluster cert reuse and giving multi-region role distinction. **Built-in CA is permanently rejected** (chicken-and-egg trust on bootstrap; turns GossamerDB into a CA with key custody / revocation responsibility — that's a different product). **SPIFFE/SPIRE and Vault** deferred to v1.x as additional `pki.Source` implementations (no API break). See PRD §11 Q8, FR-6, FR-13, FR-16, NFR-SEC-3.
- ~~**Multi-region semantics.**~~ Resolved 2026-04-29 — **Cluster-level active-passive with optional async cross-region replication; configurable per cluster.** Two modes, picked at cluster bootstrap: **`cross_region.mode = none`** (single-region only — small-team default; no WAN traffic) or **`active_passive`** (one **primary region** owns writes; **N passive regions** receive async replication, lag target < 2 s p99 under ≤ 100 ms inter-region RTT). **Topology: 1 primary + N passives** (covers global DR shapes — US + EU + APAC). **Passive-region read semantics:** server accepts `consistency=ONE` reads locally (eventual, up to ~2 s stale) and rejects `QUORUM`/`ALL` reads with **`WRONG_REGION(primary=<region>)`**. **Writes against passive regions** also return `WRONG_REGION`; the **smart Go SDK transparently retries against primary** (one cross-region hop) — symmetric with `WRONG_OWNER` handling. Non-Go REST/gRPC callers see the explicit error and choose. Cross-region traffic is observable via `cross_region_writes_total` and `cross_region_replication_lag_seconds` metrics with reference Grafana panels. **Failover is manual only in v1**: `gossamerctl promote --new-primary=<region>` flips the cluster, old primary is fenced (refuses writes when it returns) until `gossamerctl reseed` wipes + resyncs from the new primary. **Active-active and automatic region failover are deferred to v1.2** (both depend on cross-region vector-clock merge correctness machinery that is not a v1 budget item). See PRD §11 Q11, FR-9, FR-13, FR-14, FR-20, FR-21, NFR-SCALE-8.
- ~~**Authentication & authorization for clients.**~~ Resolved 2026-04-29 — **mTLS identity is the only auth surface in v1.** Admin role identified by SAN match `admin.<cluster>` (cluster-name embedded — see Q8). Per-key / per-namespace authorization (RBAC, ACLs) deferred to v1.1 alongside multi-tenant namespace isolation (Q18). **Forward-compat hook lands in v1:** a `principal` value is derived once from the client cert SAN at TLS handshake, pinned to the gRPC connection context, threaded through every handler, and emitted on every audit-log line (FR-16) and OTel span. v1.1 RBAC plugs a Policy Decision Point in front of the same field — zero wire-protocol change. Capability tokens / JWT are explicitly **rejected for v1** because JWKS distribution + per-RPC verification + revocation cache cost would compete with the < 1 ms GET p99 budget. See PRD §11 Q9, FR-6, FR-16, FR-20, NFR-SEC-4.
- ~~**Compliance posture.**~~ Resolved 2026-04-29 — split into two parts. **(a) Certifications: none in v1.** SOC2 Type II takes 6+ months of operating evidence, FedRAMP needs an agency sponsor, HIPAA is enforced at the application/BAA layer (an infra KV store cannot be HIPAA-compliant standalone). Public positioning is **"audit-ready, not audited."** A `docs/security/controls.md` ships in v1 mapping existing controls to the SOC2 CC-series so a future audit does not start from zero. **(b) Data residency in multi-region AWS: the cluster — not the row — is the residency primitive (NFR-SEC-7, §6.5).** A v1 cluster lives in one region by default (`cross_region.mode = none`); cross-region replication is opt-in via `cross_region.mode = active_passive` and an explicit `passive_regions` list. GDPR right-to-erasure is the operator's data-model responsibility; v1 supplies `Delete(key)` and audit attribution as the primitives. FedRAMP / GovCloud is out of scope until ICP demand exists. See PRD §11 Q10, NFR-SEC-5, NFR-SEC-7, §6.5, NG9, NG10, OD-2.
- ~~**Rolling-upgrade contract.**~~ Resolved 2026-04-29 (already locked in PRD) — **N / N+1 minor-version skew** is supported; mismatched-major nodes refuse to join; gossip and conflict-strategy versions are carried in gossip messages. N / N+2 skew explicitly rejected on test-cost grounds. See PRD §11 Q12, FR-10.
- ~~**Edge deployment shape.**~~ Resolved 2026-04-29 (already locked in PRD) — **edge nodes are full data nodes** running with a separate region tag. **No thin proxy or read-through cache shape in v1**; that violates the "identical binary across modes" goal (NFR-PORT-1). Read-through edge cache deferred to v1.x. See PRD §11 Q15, NFR-PORT-1.
- ~~**Public API stability commitment.**~~ Resolved 2026-04-29 (already locked in PRD) — **gRPC and REST surfaces follow semver at the protocol level** (additive-only within a major; no field-number reuse). Strategy extension points (`gossip.Strategy`, `conflict.Resolver`, `storage.Backend`) follow Go module semver — additive interface methods become a major bump. See PRD §11 Q19, NFR-API-1, NFR-API-2.
- ~~**Tooling & migration path from etcd / Consul / Redis / Dynamo.**~~ Resolved 2026-04-29 (already locked in PRD) — **none in v1; clean-slate adoption.** Importers are a v1.x deliverable (FC-7). See PRD §11 Q20.

### 9.2 Still open

_All wiki §9 questions resolved as of 2026-04-29. The PRD's §11 resolution table and §12 outstanding-decisions list are the single source of truth from this point on. Any new question raised during HLD / LLD work is recorded directly in the PRD; this list stays empty unless a new product-level question opens upstream of the PRD._
