...
- **FR-7. Coordinator HA (control plane).** The Coordinator runs as a **3-node embedded-Raft group** owning cluster metadata (membership, partition map, strategy config, rolling-upgrade orchestration). **Strictly control-plane** — never on the per-request data path (see FR-8). Coordinator failure pauses control-plane mutations only; foreground reads and writes continue uninterrupted.
