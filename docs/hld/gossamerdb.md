...
**Status:** Draft v0.1 — initial scaffold off PRD v1.13 (`docs/prds/README.md`, APPROVED 2026-05-03).
...
The PRD §12 outstanding decisions (OD-3, OD-5, OD-6, OD-7, OD-2-marketing) carry into the HLD verbatim with `needs-by` tags. The HLD adds the following architectural questions that are answer-needed before the LLD locks down corresponding interfaces:

| #      | Question                                                                                                                                                                | Default proposal (HLD)                                                                                                                                           | Needs-by   |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| OQ-1   | **Raft library choice:** `hashicorp/raft` vs `etcd-io/raft`?                                                                                                            | **`hashicorp/raft`** — better snapshot/restore ergonomics for the FR-7 cadence; cleaner mTLS hookpoints; long-running maintenance. LLD makes the final call.     | pre-LLD    |
| OQ-2   | **Wire encoding for the partition map:** group-varint deltas + zigzag-varint replicas (HLD assumption — yields ≤ 192 KiB at 128 nodes / 16 384 vnodes), or a denser proto-buf custom encoding? | Default to group-varint deltas + zigzag-varint. Re-evaluate only if `BenchmarkPartitionMapSerialization` shows > 256 KiB or > 5 ms cost.                          | pre-LLD    |
| OQ-3   | **Sharded in-memory store: how many shards per data node?**                                                                                                             | Default `runtime.NumCPU() × 4` shards; LLD benchmarks against 1×, 2×, 4×, 8× and locks the value with `BenchmarkPutShardedThroughput` evidence.                   | pre-LLD    |
| OQ-4   | **gRPC connection pool size to peer owners (HR2):** static or auto-sized?                                                                                               | Default `min(N-1, ulimit/64)` per data node; LLD locks idle policy and re-warm trigger on epoch bump.                                                            | pre-LLD    |
| OQ-5   | **Anti-entropy schedule jitter:** uniform or exponential backoff under load?                                                                                            | Default uniform jitter ±50% of cadence; LLD revisits if `BenchmarkAntiEntropyUnderLoad` shows pathological co-scheduling.                                          | pre-LLD    |
| OQ-6   | **Plumtree fanout:** static `log(N)` or adaptive on observed message-loss rate?                                                                                         | Default static `log(N)` rounded up; adaptive deferred to v1.2 alongside HyParView.                                                                                | pre-LLD    |
| OQ-7   | **Cache-instance Redis vs in-memory only at small cluster sizes:** when does the cross-instance Redis cache start paying for itself?                                     | Default: ship Redis cache always-on; small clusters can run a single-replica Redis. LLD names the threshold below which operators may disable the cross-instance cache. | pre-LLD    |
| OQ-8   | **Multi-region replication transport:** Plumtree-only when enabled, or always SWIM-piggyback for cross-region (lower jitter)?                                            | Default: follow the active gossip layer chain — Plumtree if enabled, SWIM piggyback otherwise. Bench-gated by cross-region-lag bench at M11.                       | pre-M11    |
| OQ-9   | **`region_epoch` propagation budget:** PRD locks p99 < 10 s on a 32-node cluster; what is the equivalent on a 128-node cluster?                                         | Linear extrapolation: ~25 s p99 at 128 nodes. LLD measures and revisits. SDK alert threshold scales accordingly.                                                  | pre-LLD    |
| OQ-10  | **Operator-facing config schema versioning:** in-band schema version on every config file, or out-of-band by binary version?                                            | Default: in-band schema version (`config_schema_version: 1`); rejecting unknown schemas at bootstrap. LLD locks the upgrade-time config-migration story.          | pre-LLD    |
